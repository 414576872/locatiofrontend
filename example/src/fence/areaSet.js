...
function getArea() {
    ...
    HG_AJAX("/position_sdk/ModularArea/Obstacle/getArea", {
        floor_id: FLOOR_ID
    },'post',function (data) {
        if (data.type == 1) {
...
function mapInit(url,map_zoom,extend,center,extent) {
    ...
    var my_mouse_position = new HG2DMap.control.mouse_position();
    ...
    MY_MAP.addControl(my_mouse_position);
...
$("#button_area_add_save").click(function () {
    ...
    HG_AJAX("/position_sdk/ModularArea/Obstacle/addArea", {
        floor_id:FLOOR_ID,
        area:NEW_AREA.area,
        name:NEW_AREA.name,
...
$("#button_area_delete_save").click(function () {
    HG_AJAX("/position_sdk/ModularArea/Obstacle/deleteArea", { id: EDIT_ID},'post',function (data) {
        if (data.type == 1) {
            getArea();
            HG_MESSAGE("删除成功");
        } else {
...
$("#button_area_basic_update_save").click(function () {
    ...
    HG_AJAX("/position_sdk/ModularArea/Obstacle/updateArea",{
        id: EDIT_ID,
        name: name,
        type: type,
...
$("#show_img").click(function () {
    HG_AJAX("/position_sdk/ModularNowInfo/NowInfo/creatObstacleAreaPic",{floor_id:FLOOR_ID},"post",function (data) {
        if(data.type == 1){
            var file_name = AJAX_URL +data.result+"?time="+Math.random();
            $("#modal_show_img img").attr("src",file_name);
